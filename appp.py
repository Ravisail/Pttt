...
Daily Close crosses ABOVE the Signal Week Close (yesterday's Daily Close
<= Signal Close AND today's Daily Close > Signal Close), with entry
executed at the NEXT trading day's Open. The stored Signal Week Close is
fixed until either a buy occurs or the signal is invalidated (Weekly
Close < Weekly EMA30, OR Weekly EMA10 < Weekly EMA30, OR the waiting
period exceeds the configured maximum). Only one signal / one trade is
tracked per stock at a time.

Exit rule (mandatory, every trade): a trade is opened with a Stop Loss
AND a Target simultaneously -- there is no other exit condition and a
trade is NEVER force-closed. Each day the trade remains open:
  1. Stop Loss check FIRST: if Daily Low <= Stop Loss Price, exit at the
     Stop Loss Price, Exit Reason = "Stop Loss".
  2. Target check SECOND: if Daily High >= Target Price, exit at the
     Target Price, Exit Reason = "Target".
Whichever condition is met first (in that priority order, same day) closes
the trade. If neither is hit, the trade stays open indefinitely -- there
is no trend-based or time-based exit of any kind.
...
on the first trading day AFTER the week closes; Stop Loss / Target are
checked using day D's own Low/High, never a future day's).
"""
...
def run_engine(daily: pd.DataFrame, weekly: pd.DataFrame, position_size: float, max_waiting_days: int,
    ...
    Entry: buy only on a true crossover of the Signal Week Close --
    `prev_close <= signal_close AND today_close > signal_close` -- executed
    at the NEXT trading day's Open (i.e. state becomes PENDING_ENTRY on the
    crossover day and is only actioned on the following iteration).

    Exit: every trade carries a mandatory Stop Loss and Target from the
    moment it opens. Checked every day the trade is open (including the
    entry day itself, using that day's own Low/High -- never a future
    day's, so this stays causal) in strict priority order:
      1. Stop Loss: Daily Low <= Stop Loss Price -> exit at Stop Loss Price.
      2. Target:    Daily High >= Target Price  -> exit at Target Price.
    There is no other exit condition (no trend/time-based exit) and a
    trade is NEVER force-closed -- if neither level is hit it stays open
    indefinitely, including past the end of the requested date range.

    A new signal is only opened on a False->True transition of SignalRaw
    relative to the immediately preceding evaluated week (checked
    explicitly here rather than relying on state == NONE alone, so the
    ...
    for i in range(n):
        ...
        # --- Step 1: action any weekly bar confirmed as of today (signal
        # generation / invalidation only -- trades are exited purely on
        # Stop Loss / Target below, never by a weekly condition) ---
        ...
        # --- Step 3: mandatory Stop Loss / Target check (Stop Loss first),
        # applies the same day a trade was just opened above too ---
        if state == STATE_IN_TRADE:
            ...
            exit_reason = None
            gap_exit = False
            if float(row["Low"]) <= trade["Stop Loss Price"]:
                if today_open <= trade["Stop Loss Price"]:
                    # Gapped down THROUGH the stop -- the stop level itself
                    # was never actually tradeable that day, so the realistic
                    # fill is the day's Open (worse than the nominal stop).
                    exit_price = today_open
                    gap_exit = True
                else:
                    exit_price = trade["Stop Loss Price"]
                exit_reason = "Stop Loss"
            elif float(row["High"]) >= trade["Target Price"]:
                if today_open >= trade["Target Price"]:
                    # Gapped up THROUGH the target -- a resting sell would
                    # fill at the (better) Open, not the nominal target.
                    exit_price = today_open
                    gap_exit = True
                else:
                    exit_price = trade["Target Price"]
                exit_reason = "Target"

            if exit_price is not None:
                filled_exit_price = exit_price * (1.0 - cost_frac)
                entry_price = trade["Entry Price"]
                profit_pct = (filled_exit_price - entry_price) / entry_price * 100.0
                ...
                trades.append({
                    ...
                    "Signal Close Price": round(trade["Signal Close Price"], 2),
                    "Breakout Date": trade["Breakout Date"],
                    "Entry Date": trade["Entry Date"],
                    "Entry Price": round(entry_price, 2),
                    "Stop Loss %": trade["Stop Loss %"],
                    "Stop Loss Price": round(trade["Stop Loss Price"], 2),
                    "Target %": trade["Target %"],
                    "Target Price": round(trade["Target Price"], 2),
                    "Exit Date": d,
                    "Exit Price": round(filled_exit_price, 2),
                    "Exit Reason": exit_reason,
                    "Gap Exit": "Yes" if gap_exit else "No",
            ...
            if waiting_days > max_waiting_days:
                ...
                crossed_above = (prev_close is None or prev_close <= signal["close"]) and row["Close"] > signal["close"]
                if crossed_above:
...
def analyze_symbol(symbol, daily_df, params):
    ...
    trades, live_state = run_engine(
        ...
        params["stop_loss_pct"], params["target_pct"],
...
def run_scanner(universe, params, fetch_names=False, history_days=None):
    ...
    for symbol, daily_df in data.items():
        ...
        if st_ in (STATE_SIGNAL_ACTIVE, STATE_PENDING_ENTRY):
            ...
            dist = (sig["close"] - last_close) / last_close * 100.0
            if st_ == STATE_PENDING_ENTRY:
                status_label = "\U0001F680 CROSSED – Buy Next Open"
                dist = 0.0  # already crossed; no meaningful "distance" remains
            elif dist <= 2:
            ...
            scanner_rows.append({
                ...
                "Signal Close Price": round(sig["close"], 2),
                "Current Close": round(last_close, 2),
                "Distance to Buy (%)": round(dist, 2),
                "Buy Status": status_label,
            ...
            open_rows.append({
                ...
                "Current Price": round(last_close, 2),
                "Stop Loss Price": round(tr["Stop Loss Price"], 2),
                "Target Price": round(tr["Target Price"], 2),
    ...
    if not scanner_df.empty:
        scanner_df = scanner_df.sort_values("Distance to Buy (%)", ascending=True).reset_index(drop=True)
        if fetch_names:
...
def apply_portfolio_constraints(candidate_trades, capital, position_size, max_open_positions):
    """
    Portfolio-level admission control on top of the (per-symbol, fully
    independent) candidate trades already generated by run_engine.

    Each symbol's own entry/exit timing is untouched -- this layer only
    decides whether capital AND an open "slot" were actually available to
    TAKE a candidate trade on its Entry Date, exactly as a real trader with
    finite capital would face. A trade that cannot be taken is skipped
    entirely (never partially taken, never delayed to a later date).

    Priority when multiple candidates compete for the same day's capital
    is FIFO by (Entry Date, Signal Date, Ticker) -- i.e. whichever weekly
    signal fired FIRST gets first claim, with the ticker as a final
    deterministic tie-break.

    Capital locked for a trade (position_size) is released -- together with
    that trade's realised Profit ₹ -- on its own Exit Date, and becomes
    available again for new entries dated that SAME day (exits are settled
    before that day's new entries are considered, maximising capital reuse).

    Returns (admitted: list[dict], skipped: list[dict]). Every item in
    `skipped` carries all of the original candidate's fields plus a
    "Skip Reason" of "Max Open Positions Reached" or "Insufficient Capital".
    """
    ...
    ordered = sorted(
        candidate_trades,
        key=lambda t: (pd.Timestamp(t["Entry Date"]), pd.Timestamp(t["Signal Date"]), t["Ticker"]),
    )

    by_entry_date = {}
    for t in ordered:
        by_entry_date.setdefault(pd.Timestamp(t["Entry Date"]), []).append(t)

    all_dates = sorted(
        set(pd.Timestamp(t["Entry Date"]) for t in ordered)
        | set(pd.Timestamp(t["Exit Date"]) for t in ordered)
    )

    available_capital = capital
    open_slots = []  # list of [exit_date, locked_capital, profit_rs]
    admitted, skipped = [], []

    for dt in all_dates:
        # 1) settle any exits due today FIRST -- frees capital/slots for reuse today
        still_open = []
        for exit_date, locked, profit in open_slots:
            if exit_date == dt:
                available_capital += locked + profit
            else:
                still_open.append((exit_date, locked, profit))
        open_slots = still_open

        # 2) admit today's candidate entries in FIFO priority order
        for t in by_entry_date.get(dt, []):
            if len(open_slots) >= max_open_positions:
                skipped.append({**t, "Skip Reason": "Max Open Positions Reached"})
            elif available_capital < position_size:
                skipped.append({**t, "Skip Reason": "Insufficient Capital"})
            else:
                available_capital -= position_size
                open_slots.append((pd.Timestamp(t["Exit Date"]), position_size, t["Profit ₹"]))
                admitted.append(t)

    return admitted, skipped


def run_backtest(universe, params, start_date, end_date, capital, max_open_positions):
    ...
    trade_log_cols = ["Ticker", "Signal Date", "Signal Close Price", "Breakout Date", "Entry Date",
                       "Entry Price", "Stop Loss %", "Stop Loss Price", "Target %", "Target Price",
                       "Exit Date", "Exit Price", "Exit Reason", "Gap Exit", "Holding Days",
                       "Profit %", "Profit ₹", "Maximum Drawdown", "Maximum Favourable Excursion",
                       "Maximum Adverse Excursion"]

    admitted_trades, skipped_trades = apply_portfolio_constraints(
        all_trades, capital, params["position_size"], max_open_positions
    )

    trade_log = pd.DataFrame(admitted_trades)
    if not trade_log.empty:
        trade_log = trade_log[trade_log_cols].sort_values("Entry Date").reset_index(drop=True)

    skipped_df = pd.DataFrame(skipped_trades)
    if not skipped_df.empty:
        skipped_df = skipped_df[trade_log_cols + ["Skip Reason"]].sort_values("Entry Date").reset_index(drop=True)
    ...
    progress.empty()
    return trade_log, open_positions_df, skipped_df, errors, len(data)
...
def compute_summary(trade_log: pd.DataFrame, capital: float):
    ...
    total = len(trade_log)
    sl_exits = int((trade_log["Exit Reason"] == "Stop Loss").sum())
    target_exits = int((trade_log["Exit Reason"] == "Target").sum())
    gap_exits = int((trade_log["Gap Exit"] == "Yes").sum())
...
max_wait_days = st.sidebar.number_input(
    "Maximum Waiting Days (Trading Days)", min_value=5, max_value=250, value=60,
    help="Counted in trading bars (daily rows with data), not calendar days — e.g. 60 trading days spans "
         "roughly 85–90 calendar days once weekends/holidays are excluded.",
)

st.sidebar.subheader("\U0001F6E1\uFE0F Risk Management (Mandatory)")
st.sidebar.caption("Every trade always has both a Stop Loss and a Target — neither can be turned off.")
stop_loss_pct = st.sidebar.number_input(
    "Stop Loss (%)", min_value=0.5, max_value=50.0, value=5.0, step=0.5,
    help="Stop Loss Price = Entry Price × (1 − StopLoss% / 100). Exit fires the moment Daily Low touches this price.",
)
target_pct = st.sidebar.number_input(
    "Target (%)", min_value=0.5, max_value=200.0, value=15.0, step=0.5,
    help="Target Price = Entry Price × (1 + Target% / 100). Exit fires the moment Daily High touches this price.",
)
...
max_open_positions = st.sidebar.number_input(
    "Maximum Open Positions", min_value=1, max_value=200, value=10,
    help="Caps concurrent trades in the Backtest. A signal that fires while all slots are full, or while "
         "remaining capital is below one Position Size, is skipped (never taken, never delayed) — shown "
         "separately in a 'Skipped Signals' table so nothing silently disappears.",
)
st.sidebar.caption(
    "Position Size is a fixed ₹ amount per trade, drawn from Capital. Backtest entries are now capital- "
    "and slot-constrained (FIFO by signal date); the live Scanner is unaffected since it only shows the "
    "current snapshot, not a capital simulation."
)
...
st.caption(
    ...
    "Buy only confirmed on a daily close crossing above the fixed Signal Week Close, executed at next "
    "day's open. Every trade carries a mandatory Stop Loss and Target — no other exit exists, and "
...
with tab_scan:
    ...
    st.markdown(
        "<span class='signal-caption'>Shows stocks with an active weekly signal awaiting a Signal Close "
        "crossover, sorted by nearest to buy first. Signals and displayed weekly EMA/Volume-ratio values are "
        "always taken from the last fully completed weekly bar (never a still-forming week); only Current "
        "Price / Distance to Buy use today's live daily close, since the crossover trigger is a daily "
        "event. Distance to Buy (%) = (Signal Close − Current Close) / <b>Current Close</b> × 100 — i.e. "
        "the % move needed from here to trigger a buy (not % of the Signal Close itself).</span>",
        unsafe_allow_html=True,
    )
...
with tab_bt:
    ...
    st.markdown(
        "<span class='signal-caption'>Signals detected only on completed weekly bars; entries execute at "
        "the next trading day's open on a Signal Close crossover. Every trade exits only on its mandatory "
        "Stop Loss or Target (Stop Loss checked first) — never force-closed; a gap that jumps straight "
        "through a level fills at that day's realistic Open, not the nominal level. Entries are also capital- "
        "and slot-constrained (see Max Open Positions in the sidebar) — a signal that can't be funded is "
        "skipped and shown separately, never silently taken anyway. Only trades whose Entry Date falls "
        "within the selected window are counted (extra history is downloaded beforehand purely to warm up "
        "the indicators).</span>",
        unsafe_allow_html=True,
    )
    ...
    if start_date >= end_date:
        ...
        if run_bt:
            with st.spinner("Running backtest..."):
                trade_log, open_positions_df, skipped_df, errors, n_downloaded = run_backtest(
                    universe_symbols, params, start_date, end_date, capital, max_open_positions
                )
            st.session_state["trade_log"] = trade_log
            st.session_state["bt_open_positions"] = open_positions_df
            st.session_state["bt_skipped"] = skipped_df
        ...
        if "trade_log" in st.session_state:
            ...
            skipped_df = st.session_state["bt_skipped"]
            errors = st.session_state["bt_errors"]
            n_downloaded = st.session_state["bt_n"]

            if not skipped_df.empty:
                with st.expander(
                    f"\u26D4 Signals Skipped — Capital / Slot Limit ({len(skipped_df)})", expanded=False
                ):
                    st.caption(
                        "These signals crossed and would have entered, but no Position Size was available "
                        "(Max Open Positions full, or remaining Capital below one Position Size) at the "
                        "time. They are NOT taken, NOT delayed, and NOT included in the trade log or "
            ...
            if trade_log.empty:
                ...
                r1b = st.columns(3)
                r1b[0].metric("Stop Loss Exits", summary["Stop Loss Exits"])
                r1b[1].metric("Target Exits", summary["Target Exits"])
                r1b[2].metric("Gap Exits", summary["Gap Exits"], help="Exits filled at the day's Open because price gapped straight through the Stop Loss/Target level.")
