    ...
    for i in range(n):
        ...
        if state == STATE_WAIT_RECOVERY:
            if (c > e21) and (c > e50) and (c > e200) and \
                ...
                # ---- CONTINUATION RULE -----------------------------------
                # If the bullish trend is STILL intact on this BUY candle,
                # do NOT reset to IDLE. Immediately start a NEW tracking
                # cycle from the BUY candle (this candle becomes the new
                # tracking start) and wait for the next pullback. The restart
                # condition is the exact same fully-bullish condition used to
                # begin a fresh tracking cycle from IDLE, so a restarted cycle
                # is indistinguishable from a fresh one downstream.
                restart = (c > e200 and c > e50 and c > e21 and
                           e21 > e200 and e50 > e200 and rsi > 70.0 and
                           (not strict_trend or fanned(e21, e50, e200)))
                if restart:
                    state = STATE_TRACKING
                    tracking = True
                    pullback_found = False
                    tracking_start_idx = i      # BUY candle -> new tracking start
                    pullback_idx = None
                    stats["tracking_started"] += 1
                    if collect_debug:
                        debug.append(DebugEvent(
                            symbol, d, "TRACKING RESTARTED",
                            "Reason: RSI still above 70; trend still valid "
                            f"(RSI {rsi:.1f}  Close {c:.2f}  EMA21 {e21:.2f} "
                            f"EMA50 {e50:.2f} EMA200 {e200:.2f}); waiting for "
                            "next pullback"))
                else:
                    # Trend no longer fully bullish on the BUY candle
                    # (RSI dropped <=70 or EMA hierarchy failed) -> reset.
                    if collect_debug:
                        debug.append(DebugEvent(
                            symbol, d, "RESET",
                            f"Trend not intact on buy candle (RSI {rsi:.1f}); "
                            "cycle closed, returning to IDLE"))
                    state = STATE_IDLE
                    tracking = False
                    pullback_found = False
                    tracking_start_idx = None
    ...
    while i < n:
        ...
        if phase == "idle":
            ...
            if (c > a21) and (c > a50) and (c > a200) and \
                ...
                # CONTINUATION RULE (independent re-derivation): if the trend
                # is still fully bullish on this buy candle, restart tracking
                # from it instead of resetting. Same condition as a fresh
                # idle -> tracking start.
                restart = (c > a200 and c > a50 and c > a21 and
                           a21 > a200 and a50 > a200 and r > 70.0 and
                           (not strict_trend or fan(i)))
                if restart:
                    phase = "tracking"
                    t_start = i          # buy candle -> new tracking start
                    pb = None
                else:
                    phase = "idle"
                    t_start = pb = None
...
class PortfolioSettings:
    ...
    position_size_pct: float      # % (of initial capital or current equity)
    ...
    max_hold_days: int
    # ---- portfolio-simulator options (all defaulted -> backward compatible) ----
    alloc_mode: str = "% of Initial Capital"   # or "% of Current Equity" / "Fixed Amount"
    fixed_amount: float = 10000.0              # used when alloc_mode == "Fixed Amount"
    priority_mode: str = "First generated"     # same-day signal execution priority
    ambiguous_mode: str = "Conservative (SL first)"  # SL & TP same candle handling
...
class Trade:
    ...
    reason: str = ""
    # ---- richer portfolio accounting fields (defaulted) ----
    quantity: int = 0
    invested: float = 0.0
    exit_reason: str = ""
    position_id: int = -1
    # rejection / skip diagnostics (populated for Skipped trades)
    cash_available: float = 0.0
    cash_required: float = 0.0
    shares_required: int = 0
    position_size: float = 0.0
    open_positions_at: int = 0
    free_slots: int = 0
...
def _last_finite_close(ind: pd.DataFrame, k: int):
    """Most recent finite StratClose at or before index k (for NaN fallbacks)."""
    for j in range(k, -1, -1):
        v = float(ind.iloc[j]["StratClose"])
        if np.isfinite(v):
            return v
    return None


def _simulate_exit(ind: pd.DataFrame, entry_idx: int, entry_price: float,
                   settings: "PortfolioSettings"):
    """Deterministic per-trade exit: SL / TP / max-hold. When BOTH the stop and
    the target are touched on the same candle the outcome follows
    settings.ambiguous_mode. Returns (exit_idx, raw_exit_price, reason). The
    caller applies exit slippage to MARKET exits only (see close_position).
    Gap-aware: a candle that opens beyond a trigger fills at the open, so stops
    are never filled better than a gap-down open and targets capture gap-ups."""
    sl_price = entry_price * (1.0 - settings.stop_loss_pct / 100.0)
    tp_price = entry_price * (1.0 + settings.take_profit_pct / 100.0)
    mode = getattr(settings, "ambiguous_mode", "Conservative (SL first)")
    n = len(ind)
    for k in range(entry_idx, n):
        op = float(ind.iloc[k]["StratOpen"])
        hi = float(ind.iloc[k]["StratHigh"])
        lo = float(ind.iloc[k]["StratLow"])
        cl = float(ind.iloc[k]["StratClose"])
        held = k - entry_idx

        # --- NaN guard: a halt / data-gap candle is non-tradable. We cannot
        #     observe a stop or target on it, so we hold through it. Max-hold can
        #     still time out here, exiting at the last finite close. ---
        if not (np.isfinite(op) and np.isfinite(hi)
                and np.isfinite(lo) and np.isfinite(cl)):
            if held >= settings.max_hold_days:
                fb = _last_finite_close(ind, k)
                return k, (fb if fb is not None else entry_price), "Max Hold"
            continue

        # --- Gap risk (skip on the entry candle, whose open is the fill ref) ---
        # If a later candle OPENS beyond a trigger, that open is the first
        # tradable price, so the realistic fill is the open -- never the ideal
        # level. This prevents impossible stop fills on gap-downs and captures
        # favourable gap-ups on targets, regardless of ambiguous_mode.
        if k > entry_idx:
            if op <= sl_price:            # gapped through the stop -> filled at open
                return k, op, "Stop Loss"
            if op >= tp_price:            # gapped through the target -> filled at open
                return k, op, "Take Profit"

        hit_sl = lo <= sl_price
        hit_tp = hi >= tp_price
        # Reaching this point means the open did NOT gap past either level, so
        # an intraday touch fills at the exact stop / target price.
        if hit_sl and hit_tp:
            if mode == "Optimistic (TP first)":
                return k, tp_price, "Take Profit"
            if mode == "Exit at candle close":
                return k, cl, "Ambiguous Close"
            if mode == "Ignore ambiguous candle":
                if held >= settings.max_hold_days:
                    return k, cl, "Max Hold"
                continue
            return k, sl_price, "Stop Loss"          # Conservative (default)
        if hit_sl:
            return k, sl_price, "Stop Loss"
        if hit_tp:
            return k, tp_price, "Take Profit"
        if held >= settings.max_hold_days:
            return k, cl, "Max Hold"
    fb = _last_finite_close(ind, n - 1)
    return n - 1, (fb if fb is not None else entry_price), "End of Data"


def _alloc_amount(settings: "PortfolioSettings", cash: float, equity: float) -> float:
    """Target rupees to deploy on the next trade for the selected sizing mode."""
    mode = getattr(settings, "alloc_mode", "% of Initial Capital")
    if mode == "Fixed Amount":
        return float(getattr(settings, "fixed_amount", 0.0))
    if mode == "% of Current Equity":
        return equity * settings.position_size_pct / 100.0
    return settings.initial_capital * settings.position_size_pct / 100.0
...
def backtest_engine(signals_by_symbol: Dict[str, List[BuySignal]],
    ...
    """
    MODULE 3 -- Institutional-grade portfolio simulator.

    Consumes the generated Buy signals ONLY (never recalculates them). Runs a
    real cash-accounted portfolio day by day across a unified trading calendar:

      * Cash is actually deducted when a position opens and the invested capital
        is reserved until the position exits; the same cash can never fund two
        simultaneous trades.
      * On exit the reserved capital is returned along with realized P&L, net of
        brokerage, taxes and slippage.
      * Max-open-positions is enforced on the count of positions that are
        genuinely open at that moment (freed the instant a trade exits).
      * Position sizing supports Fixed Amount, % of Initial Capital and
        % of Current Equity.
      * Same-day competing entries are ordered by the chosen priority; if
        capital/slots are limited the higher-priority trades fill first.
      * A daily equity curve (cash + market value of open positions) is produced
        for drawdown / exposure / monthly / annual reporting.

    Signal-selection, rejection reasons and the entry rule (next-day OPEN) are
    unchanged; exit rules are unchanged apart from the configurable
    ambiguous-candle policy.

    Simulation assumptions (explicit modelling conventions, not an exchange
    matching engine; deterministic and applied uniformly to every trade):
      * Entry fills at the NEXT trading day's OPEN after the buy signal
        (slippage-adjusted); exits fill at the SL/TP level or the candle close.
      * Each day is processed as (1) exits for positions opened on earlier days,
        (2) today's new entries in priority order, (3) same-day exits for
        positions opened today (possible via ambiguous-candle / max-hold rules).
      * Capital freed by a step-(1) exit is available to fund today's entries;
        capital freed in step (3) frees for the next day, not the same day.
      * %-of-equity sizing uses ONE start-of-day equity snapshot for all of that
        day's entries, so allocation is independent of processing order.

    Returns (trades, summary); summary carries the daily capital timeline for
    the reporting layer.
    """
    import random as _random
    _rng = _random.Random(20240517)

    brok = settings.brokerage_pct / 100.0
    slip = settings.slippage_pct / 100.0
    tax = settings.taxes_pct / 100.0

    # ---- generation order (for "First generated" priority) ----
    gen_order = [s for sigs in signals_by_symbol.values() for s in sigs]
    gen_index = {id(s): n for n, s in enumerate(gen_order)}
    events = sorted(gen_order, key=lambda s: (s.buy_date, s.symbol))

    all_trades: List[Trade] = []      # one row per signal, in events order
    candidates: List[Dict] = []       # survivors that will attempt entry
    seen_keys = set()
    ...
    for s in events:
        ...
        if verified_keys is not None and key not in verified_keys:
        ...
        if entry_idx is None:
            t.status, t.reason = "Rejected", "No next trading day"
            continue

        entry_price = float(ind.iloc[entry_idx]["StratOpen"]) * (1.0 + slip)
        if not np.isfinite(entry_price) or entry_price <= 0:
            t.status, t.reason = "Rejected", "Invalid entry price"
            continue
        ex_idx, ex_raw, ex_reason = _simulate_exit(ind, entry_idx, entry_price, settings)
        candidates.append(dict(
            signal=s, key=key, trade=t, ind=ind,
            entry_idx=entry_idx, entry_date=str(ind.index[entry_idx].date()),
            entry_price=entry_price, exit_idx=ex_idx, exit_raw=ex_raw,
            exit_reason=ex_reason, gen=gen_index[id(s)], rand=_rng.random()))

    # ---------- chronological portfolio simulation ----------
    per_symbol_dateidx = {
        sym: {ts.date(): k for k, ts in enumerate(ind.index)}
        for sym, ind in data_by_symbol.items()}
    ...
    def prio_key(c):
        s = c["signal"]
        m = getattr(settings, "priority_mode", "First generated")
        dist = ((s.signal_price - s.ema200) / s.ema200) if s.ema200 else 0.0
        if m == "Highest RSI":
            return (-s.rsi,)
        if m == "Lowest RSI":
            return (s.rsi,)
        if m == "Highest EMA200 distance":
            return (-dist,)
        if m == "Lowest EMA200 distance":
            return (dist,)
        if m == "Alphabetical":
            return (s.symbol,)
        if m == "Random":
            return (c["rand"],)
        return (c["gen"],)            # First generated
    ...
    timeline: List[Dict] = []
    pos_seq = 0
    all_pos_ids: List[int] = []
    stale_days: Dict[str, int] = {}
    max_mtm_stale = 0

    def close_position(p, dstr):
        nonlocal cash, realized
        # Take Profit is a LIMIT order (fills at price or better) -> no adverse
        # slippage. Stop Loss / Max Hold / Ambiguous Close / End of Data / gap
        # fills are MARKET orders -> apply slippage.
        is_limit = (p["exit_reason"] == "Take Profit")
        exit_net = p["exit_raw"] if is_limit else p["exit_raw"] * (1.0 - slip)
        gross = p["shares"] * exit_net
        exit_fees = gross * brok
        taxes = max(0.0, gross - p["cost"]) * tax
        proceeds = gross - exit_fees - taxes
        pnl = proceeds - p["cost"] - p["entry_fees"]
        cash += proceeds
        realized += pnl
        t = p["trade"]
        t.entry_date = p["entry_date"]
        t.entry_price = round(p["entry_price"], 4)
        t.exit_date = dstr
        t.exit_price = round(exit_net, 4)
        t.pnl = round(pnl, 2)
        t.ret_pct = round((exit_net / p["entry_price"] - 1.0) * 100.0, 2)
        t.holding_days = int(p["exit_idx"] - p["entry_idx"])
        t.status = "Executed"
        t.exit_reason = p["exit_reason"]
        t.quantity = int(p["shares"])
        t.invested = round(p["cost"], 2)
        t.position_id = p["pos_id"]

    if candidates:
        first_entry = min(c["entry_date"] for c in candidates)
        last_exit = max(str(c["ind"].index[c["exit_idx"]].date()) for c in candidates)
        all_days = sorted({d for dm in per_symbol_dateidx.values() for d in dm})
        days = [d for d in all_days if first_entry <= str(d) <= last_exit]
    else:
        days = []
    ...
    for D in days:
        ...
        # (1) exits for positions opened on an earlier day
        keep = []
        for p in open_positions:
            if p["exit_date"] == Dstr and p["entry_date"] < Dstr:
                close_position(p, Dstr)
            else:
                keep.append(p)
        open_positions = keep

        # Cash available to fund TODAY's entries = cash after prior-day exits,
        # before any of today's entries or same-day exits (which free capital
        # only for the next day). Reported separately from end-of-day cash.
        avail_cash = cash

        # (2) entries scheduled for today, in priority order.
        # Size EVERY same-day entry from one start-of-day equity snapshot,
        # taken after prior-day exits and before any of today's entries, so that
        # %-of-equity sizing does not depend on the order the day's trades are
        # processed in (earlier entries no longer shrink later ones' allocation).
        day_equity = cash + sum(
            p["shares"] * last_close.get(p["symbol"], p["entry_price"])
            for p in open_positions)
        for c in sorted(entries_by_date.get(Dstr, []), key=prio_key):
            t = c["trade"]
            free_slots = settings.max_open_positions - len(open_positions)

            if len(open_positions) >= settings.max_open_positions:
                t.status, t.reason = "Skipped", "Max positions reached"
                t.cash_available = round(cash, 2)
                t.open_positions_at = len(open_positions)
                t.free_slots = 0
                continue

            alloc = _alloc_amount(settings, cash, day_equity)
            ep = c["entry_price"]
            shares = int(alloc // ep) if ep > 0 else 0
            cost = shares * ep
            entry_fees = cost * brok
            required = cost + entry_fees

            if shares <= 0 or required > cash:
                t.status, t.reason = "Skipped", "Insufficient Capital"
                t.cash_available = round(cash, 2)
                t.cash_required = round(required, 2)
                t.shares_required = int(shares)
                t.position_size = round(alloc, 2)
                t.open_positions_at = len(open_positions)
                t.free_slots = int(free_slots)
                continue

            cash -= required
            pos_seq += 1
            all_pos_ids.append(pos_seq)
            open_positions.append(dict(
                pos_id=pos_seq, trade=t, symbol=c["signal"].symbol,
                shares=shares, cost=cost, entry_fees=entry_fees,
                entry_idx=c["entry_idx"], entry_date=c["entry_date"],
                entry_price=ep, exit_idx=c["exit_idx"], exit_raw=c["exit_raw"],
                exit_date=str(c["ind"].index[c["exit_idx"]].date()),
                exit_reason=c["exit_reason"]))

        # (3) same-day exits for positions opened today
        keep = []
        for p in open_positions:
            if p["exit_date"] == Dstr and p["entry_date"] == Dstr:
                close_position(p, Dstr)
        ...
        # (4) mark-to-market and record the daily capital timeline.
        # Track MTM staleness: how many consecutive days each open position's
        # symbol has gone without a fresh candle (halt / delist / internal gap).
        for sym, dm in per_symbol_dateidx.items():
            if D in dm:
                cval = float(data_by_symbol[sym].iloc[dm[D]]["StratClose"])
                if np.isfinite(cval):        # ignore NaN halt closes; keep last good
                    last_close[sym] = cval
        for p in open_positions:
            if D in per_symbol_dateidx.get(p["symbol"], {}):
                stale_days[p["symbol"]] = 0
            else:
                stale_days[p["symbol"]] = stale_days.get(p["symbol"], 0) + 1
                max_mtm_stale = max(max_mtm_stale, stale_days[p["symbol"]])
        invested = sum(p["cost"] for p in open_positions)
        mv = sum(p["shares"] * last_close.get(p["symbol"], p["entry_price"])
                 for p in open_positions)
        # Round components first, then derive Equity from them so that the
        # identity  Equity == Cash + Market Value  holds exactly at 2dp.
        cash_r = round(cash, 2)
        mv_r = round(mv, 2)
        inv_r = round(invested, 2)
        timeline.append({
            "Date": Dstr,
            "Cash": cash_r,
            "Available Cash": round(avail_cash, 2),
            "Invested": inv_r,
            "Market Value": mv_r,
            "Equity": round(cash_r + mv_r, 2),
            "Open Positions": len(open_positions),
            "Unrealized P&L": round(mv_r - inv_r, 2),
            "Realized P&L": round(realized, 2),
        })

    # safety: force-close anything still open at the last available candle
    for p in list(open_positions):
        ind = data_by_symbol[p["symbol"]]
        close_position(p, str(ind.index[p["exit_idx"]].date()))
    open_positions = []

    daily_df = pd.DataFrame(timeline) if timeline else pd.DataFrame(columns=[
        "Date", "Cash", "Available Cash", "Invested", "Market Value", "Equity",
        "Open Positions", "Unrealized P&L", "Realized P&L"])

    summary = dict(
        final_cash=round(cash, 2),
        total_return_pct=round((cash / settings.initial_capital - 1.0) * 100.0, 2),
        executed=sum(1 for t in all_trades if t.status == "Executed"),
        skipped=sum(1 for t in all_trades if t.status == "Skipped"),
        rejected=sum(1 for t in all_trades if t.status == "Rejected"),
        initial_capital=float(settings.initial_capital),
        realized_pnl=round(realized, 2),
        max_mtm_staleness=int(max_mtm_stale),
        daily=daily_df,
...
# ==============================================================================
# PORTFOLIO REPORTING  (derived purely from the simulator output; no signals)
# ==============================================================================
def _underwater(equity: List[float]):
    """Return (underwater_pct_list, max_drawdown_pct, longest_drawdown_days)."""
    if not equity:
        return [], 0.0, 0
    peak = equity[0]
    last_peak_i = 0
    ...
    for i, e in enumerate(equity):
        if e >= peak:
            peak = e
            longest = max(longest, i - last_peak_i)
            last_peak_i = i
        u = ((e - peak) / peak * 100.0) if peak > 0 else 0.0
        uw.append(round(u, 3))
        maxdd = min(maxdd, u)
    longest = max(longest, (len(equity) - 1) - last_peak_i)
    return uw, round(abs(maxdd), 2), int(longest)


def _safe_agg(fn, arr, default=0.0):
    """np.nan* aggregation that returns `default` for empty / all-NaN input
    instead of emitting a RuntimeWarning and NaN."""
...
def portfolio_stats(summary: Dict, trades: List[Trade]) -> Dict[str, object]:
    """Capital-deployment / cash statistics from the daily timeline."""
    d = summary.get("daily")
    ex = [t for t in trades if t.status == "Executed"]
    out: Dict[str, object] = {}
    if d is None or len(d) == 0:
        return out
    eq = d["Equity"].to_numpy(dtype=float)
    inv = d["Invested"].to_numpy(dtype=float)
    csh = d["Cash"].to_numpy(dtype=float)
    opn = d["Open Positions"].to_numpy(dtype=float)
    safe_eq = np.where(eq == 0, np.nan, eq)
    out["Maximum Capital Deployed"] = round(float(inv.max()), 2)
    out["Average Capital Deployed"] = round(float(inv.mean()), 2)
    out["Average Cash Balance"] = round(float(csh.mean()), 2)
    out["Average Open Positions"] = round(float(opn.mean()), 2)
    out["Maximum Open Positions"] = int(opn.max())
    out["Capital Utilization %"] = round(_safe_agg(np.nanmean, inv / safe_eq) * 100.0, 2)
    out["Idle Cash %"] = round(_safe_agg(np.nanmean, csh / safe_eq) * 100.0, 2)
    sizes = [t.invested for t in ex if t.invested > 0]
    out["Average Trade Size"] = round(float(np.mean(sizes)), 2) if sizes else 0.0
    out["Largest Trade"] = round(float(max(sizes)), 2) if sizes else 0.0
    out["Smallest Trade"] = round(float(min(sizes)), 2) if sizes else 0.0
    out["Max MTM Staleness (days)"] = int(summary.get("max_mtm_staleness", 0))
    return out


def drawdown_report(summary: Dict) -> Dict[str, object]:
    d = summary.get("daily")
    if d is None or len(d) == 0:
        return dict(underwater=[], dates=[], max_dd=0.0, longest=0, equity=[])
    eq = [float(x) for x in d["Equity"].tolist()]
    uw, maxdd, longest = _underwater(eq)
    return dict(underwater=uw, dates=list(d["Date"]), max_dd=maxdd,
                longest=longest, equity=eq)


def _period_table(summary: Dict, trades: List[Trade], key_len: int) -> pd.DataFrame:
    """Monthly (key_len=7) or annual (key_len=4) performance table."""
    d = summary.get("daily")
    cols = ["Period", "Starting Capital", "Ending Capital", "Return %",
            "Trades", "Win Rate %", "Profit", "Loss", "Max Drawdown %"]
    if d is None or len(d) == 0:
        return pd.DataFrame(columns=cols)
    ex = [t for t in trades if t.status == "Executed"]
    rows = []
    d = d.copy()
    d["P"] = d["Date"].str.slice(0, key_len)
    for per, g in d.groupby("P", sort=True):
        start = float(g["Equity"].iloc[0])
        end = float(g["Equity"].iloc[-1])
        eqs = [float(x) for x in g["Equity"].tolist()]
        _, mdd, _ = _underwater(eqs)
        tr = [t for t in ex if t.entry_date[:key_len] == per]
        wins = [t for t in tr if t.pnl > 0]
        profit = sum(t.pnl for t in tr if t.pnl > 0)
        loss = sum(t.pnl for t in tr if t.pnl <= 0)
        rows.append({
            "Period": per,
            "Starting Capital": round(start, 2),
            "Ending Capital": round(end, 2),
            "Return %": round((end / start - 1.0) * 100.0, 2) if start > 0 else 0.0,
            "Trades": len(tr),
            "Win Rate %": round(100.0 * len(wins) / len(tr), 1) if tr else 0.0,
...
def monthly_table(summary, trades):
    return _period_table(summary, trades, 7)


def annual_table(summary, trades):
    return _period_table(summary, trades, 4)


def exposure_report(summary: Dict, settings: PortfolioSettings) -> Dict[str, object]:
    d = summary.get("daily")
    out: Dict[str, object] = {}
    if d is None or len(d) == 0:
        return out
    ...
    opn = d["Open Positions"].to_numpy(dtype=float)
    safe_eq = np.where(eq == 0, np.nan, eq)
    expo = inv / safe_eq * 100.0
    out["Average Exposure %"] = round(_safe_agg(np.nanmean, expo), 2)
    out["Maximum Exposure %"] = round(_safe_agg(np.nanmax, expo), 2)
    out["Minimum Exposure %"] = round(_safe_agg(np.nanmin, expo), 2)
    mx = settings.max_open_positions
    out["Days Fully Invested"] = int(np.sum(opn >= mx))
    out["Days Partially Invested"] = int(np.sum((opn > 0) & (opn < mx)))
    out["Days Fully in Cash"] = int(np.sum(opn == 0))
    return out


def extended_trade_analytics(trades: List[Trade], summary: Dict) -> Dict[str, object]:
    ex = [t for t in trades if t.status == "Executed"]
    out: Dict[str, object] = {}
    if not ex:
        return out
    wins = [t.pnl for t in ex if t.pnl > 0]
    losses = [t.pnl for t in ex if t.pnl <= 0]
    holds = [t.holding_days for t in ex]
    net = sum(t.pnl for t in ex)
    gp = sum(wins)
    gl = -sum(losses)
    out["Average Winner"] = round(float(np.mean(wins)), 2) if wins else 0.0
    out["Average Loser"] = round(float(np.mean(losses)), 2) if losses else 0.0
    out["Largest Winner"] = round(float(max(wins)), 2) if wins else 0.0
    out["Largest Loser"] = round(float(min(losses)), 2) if losses else 0.0
    out["Median Winner"] = round(float(np.median(wins)), 2) if wins else 0.0
    out["Median Loser"] = round(float(np.median(losses)), 2) if losses else 0.0
    out["Average Holding Days"] = round(float(np.mean(holds)), 1)
    out["Longest Holding"] = int(max(holds))
    out["Shortest Holding"] = int(min(holds))
    out["Profit Factor"] = round(gp / gl, 2) if gl > 0 else float("inf")
    dd = drawdown_report(summary)
    mdd_val = summary["initial_capital"] * dd["max_dd"] / 100.0
    out["Recovery Factor"] = round(net / mdd_val, 2) if mdd_val > 0 else float("inf")
    out["Expectancy ($/trade)"] = round(net / len(ex), 2)
    rets = np.array([t.ret_pct / 100.0 for t in ex], dtype=float)
    sd = rets.std(ddof=1) if len(rets) > 1 else 0.0
    out["SQN"] = round(float(np.sqrt(len(ex)) * rets.mean() / sd), 2) if sd > 0 else 0.0
    win_rate = len(wins) / len(ex)
    avg_w = np.mean(wins) if wins else 0.0
    avg_l = abs(np.mean(losses)) if losses else 0.0
    if avg_l > 0:
        rr = avg_w / avg_l
        kelly = win_rate - (1.0 - win_rate) / rr if rr > 0 else 0.0
    else:
        kelly = win_rate
    out["Kelly %"] = round(float(kelly) * 100.0, 2)
    return out


def validation_checks(summary: Dict, trades: List[Trade],
                      settings: PortfolioSettings) -> List[Dict[str, str]]:
    """Self-tests that the portfolio accounting is internally consistent."""
    d = summary.get("daily")
    ex = [t for t in trades if t.status == "Executed"]
    ...
    if d is not None and len(d):
        ...
        add("Cash never negative", min_cash >= -1e-6, f"min cash = {min_cash:,.2f}")
        viol = int((d["Invested"] > d["Equity"] + 1e-6).sum())
        add("Invested never exceeds equity", viol == 0, f"{viol} day(s) violating")
        max_open = int(d["Open Positions"].max())
        add("Open positions <= limit", max_open <= settings.max_open_positions,
            f"peak {max_open} / limit {settings.max_open_positions}")
        recon = float((d["Equity"] - d["Cash"] - d["Market Value"]).abs().max())
        add("Daily equity == Cash + Open Value", recon < 1e-4,
            f"max reconciliation error = {recon:.6f}")
    else:
        add("Daily timeline present", len(ex) == 0, "no executed trades")

    keys = [(t.symbol, t.tracking, t.pullback, t.buy_signal) for t in ex]
    add("No duplicate executed trades", len(keys) == len(set(keys)),
        f"{len(keys) - len(set(keys))} duplicate(s)")
    pids = [t.position_id for t in ex]
    add("No duplicate position IDs", len(pids) == len(set(pids)),
        f"{len(pids) - len(set(pids))} duplicate(s)")
    one_exit = all(t.entry_date and t.exit_date for t in ex)
    add("Every entry has exactly one exit", one_exit,
        "all executed trades have entry & exit dates" if one_exit
        else "missing entry/exit date")
    if d is not None and len(d):
        final_invested = float(d["Invested"].iloc[-1])
        add("All reserved capital returned", abs(final_invested) < 1e-6,
            f"residual invested = {final_invested:,.2f}")
        # cash-flow reconciliation: final cash == initial + realized P&L
        realized = summary.get("realized_pnl", sum(t.pnl for t in ex))
        expected = round(settings.initial_capital + realized, 2)
        add("Cash flow reconciles with P&L",
            abs(summary["final_cash"] - expected) < 1e-2,
            f"final {summary['final_cash']:,.2f} vs expected {expected:,.2f}")
    return checks
...
def compute_analytics(trades: List[Trade], signals: List[BuySignal],
    ...
    # Equity curve ordered by exit date (realized). Tie-break deterministically
    # on (entry_date, symbol) so same-day exits produce a reproducible path and
    # drawdown/streak metrics are order-independent.
    ordered = sorted(ex, key=lambda t: (t.exit_date, t.entry_date, t.symbol))
...
def current_pending(symbol: str, ind: pd.DataFrame, strict_trend: bool = True):
    ...
    for i in range(len(ind)):
        ...
        if state == STATE_IDLE:
            ...
            if (c > e21 and c > e50 and c > e200
                    and (not strict_trend or fanned(e21, e50, e200))):
                # Continuation rule: restart tracking from the buy candle if
                # the trend is still fully bullish, otherwise reset to idle.
                if (c > e200 and c > e50 and c > e21 and e21 > e200
                        and e50 > e200 and rsi > 70.0
                        and (not strict_trend or fanned(e21, e50, e200))):
                    state = STATE_TRACKING; t_start = i; pb = None
                else:
...
def audit_signal(s: BuySignal, ind: pd.DataFrame, verified: bool,
                 duplicate: bool, strict_trend: bool = True,
                 prev_buy_date: Optional[str] = None) -> Dict[str, str]:
    """Return PASS/FAIL for every audit rule for a single signal.

    ``prev_buy_date`` is the buy date of the immediately preceding signal for
    the SAME symbol (or None if this is that symbol's first signal). It is used
    by the continuation-cycle checks: a restarted cycle must have its tracking
    start equal to the previous buy date, with a genuine pullback in between.
    """
    def P(x: bool) -> str:
    ...
    # ---- CONTINUATION-CYCLE AUDIT (new tracking-restart logic) -----------
    # A signal is a "restarted" cycle when its tracking start equals the
    # previous signal's buy date for the same symbol. Fresh cycles (first
    # signal of a symbol, or a cycle that began after a trend-break reset)
    # are marked N/A for the restart-specific checks -- N/A never fails.
    is_restart = (prev_buy_date is not None
                  and s.tracking_start_date == prev_buy_date)
    NA = "N/A"
    checks["Restarted Tracking after Buy"] = (
        "PASS" if is_restart else NA)
    checks["New Tracking Date == previous Buy Date"] = (
        P(s.tracking_start_date == prev_buy_date) if is_restart else NA)
    # Cycle-level uniqueness: no second Buy on the same tracking cycle.
    checks["No duplicate Buy on same cycle"] = P(not duplicate)
    # There must be a genuine pullback strictly between tracking start and
    # buy -- guarantees no back-to-back Buy without a fresh pullback.
    checks["No missing pullback between Buys"] = P(
        s.tracking_start_date < s.pullback_date < s.buy_date)
...
# STREAMLIT UI
# ==============================================================================
# --- pandas Styler cell-styling shim (Styler.map added in 2.1; applymap
#     deprecated there and removed in 3.0). Use .map when present. ---
def _cellstyle(styler, func, **kw):
    fn = getattr(styler, "map", None)
    return (fn or styler.applymap)(func, **kw)
...
with st.sidebar:
    ...
    alloc_mode = st.radio(
        "Capital Allocation",
        ["% of Initial Capital", "% of Current Equity", "Fixed Amount"],
        index=0,
        help="How much to deploy per trade. The two %-modes use the "
             "'Position Size (%)' above; 'Fixed Amount' uses the rupee value below.")
    fixed_amount = st.number_input(
        "Fixed Amount per trade", 0.0, 1e9, 10000.0, step=1000.0,
        help="Used only when Capital Allocation is 'Fixed Amount'.")
    ...
    priority_mode = st.selectbox(
        ...
        ["First generated", "Highest RSI", "Lowest RSI",
         "Highest EMA200 distance", "Lowest EMA200 distance",
         "Alphabetical", "Random"],
        index=0,
        help="When several signals share the same entry day and capital/slots "
             "are limited, higher-priority trades fill first.")
    ambiguous_mode = st.selectbox(
        "Ambiguous candle (SL & TP same day)",
        ["Conservative (SL first)", "Optimistic (TP first)",
         "Ignore ambiguous candle", "Exit at candle close"],
        index=0,
        help="Outcome when a single candle touches both the stop and the target.")
...
# The heavy work (data download + signal generation) is cached in run_all;
# the backtest itself is cheap, so run it LIVE against the current sidebar
# settings. This lets the Backtest/Analytics tabs react instantly to changes
# in stop-loss, sizing, priority, etc. without re-clicking "Run".
trades, bt_summary = backtest_engine(
    results["engine_by_symbol"], results["data"], verified_keys, settings)
...
with tabs[2]:
    ...
    if trades:
        ...
        # ===================================================================
        # INSTITUTIONAL PORTFOLIO REPORTING (portfolio simulator output only)
        # ===================================================================
        def _kv_table(d, kcol="Metric", vcol="Value"):
            return pd.DataFrame(
                [{kcol: k, vcol: ("inf" if v == float("inf") else v)}
                 for k, v in d.items()])

        daily_df = bt_summary.get("daily")

        with st.expander("Simulation assumptions", expanded=False):
            st.markdown(
                "- **Entry** fills at the next trading day's **open** after the "
                "buy signal (slippage-adjusted); **exit** fills at the SL/TP "
                "level or the candle close.\n"
                "- Each day runs as **(1)** exits for positions opened earlier, "
                "**(2)** today's entries in priority order, **(3)** same-day "
                "exits for positions opened today.\n"
                "- Capital freed in step (1) **can** fund today's entries; "
                "capital freed in step (3) frees for the **next** day.\n"
                "- **% of Current Equity** sizing uses one **start-of-day equity "
                "snapshot** for every entry that day, so sizing is independent of "
                "processing order.\n"
                "- When one candle touches **both** stop and target, the outcome "
                "follows the **Ambiguous candle** setting.")

        # ---- Rejection & Skip diagnostics ----
        with st.expander("Rejection & Skip Details", expanded=False):
            rej = [t for t in trades if t.status in ("Skipped", "Rejected")]
            if rej:
                rdf = pd.DataFrame([{
                    "Symbol": t.symbol, "Buy Signal": t.buy_signal,
                    "Status": t.status, "Reason": t.reason,
                    "Cash Available": t.cash_available,
                    "Cash Required": t.cash_required,
                    "Shares Required": t.shares_required,
                    "Position Size": t.position_size,
                    "Open Positions": t.open_positions_at,
                    "Free Slots": t.free_slots,
                } for t in rej])
                st.dataframe(rdf, use_container_width=True, hide_index=True)
                st.download_button("Download Rejections (CSV)",
                                   rdf.to_csv(index=False).encode(),
                                   "rejections.csv", "text/csv")
            else:
                st.caption("No skipped or rejected trades.")

        # ---- Portfolio statistics ----
        pstats = portfolio_stats(bt_summary, trades)
        if pstats:
            st.markdown("### Portfolio Statistics")
            cA, cB, cC = st.columns(3)
            cA.metric("Max Capital Deployed", f"{pstats['Maximum Capital Deployed']:,.0f}")
            cB.metric("Avg Capital Deployed", f"{pstats['Average Capital Deployed']:,.0f}")
            cC.metric("Capital Utilization %", pstats["Capital Utilization %"])
            st.dataframe(_kv_table(pstats), use_container_width=True, hide_index=True)

        # ---- Daily equity curve, drawdown & underwater ----
        if daily_df is not None and len(daily_df):
            st.markdown("### Daily Equity Curve (Cash + Open Position Value)")
            eqdf = daily_df.set_index("Date")[["Equity", "Cash", "Invested"]]
            st.line_chart(eqdf, use_container_width=True)

            dd = drawdown_report(bt_summary)
            dcol1, dcol2 = st.columns(2)
            dcol1.metric("Max Drawdown %", dd["max_dd"])
            dcol2.metric("Longest Drawdown (days)", dd["longest"])
            uw_df = pd.DataFrame({"Date": dd["dates"],
                                  "Underwater %": dd["underwater"]}).set_index("Date")
            st.markdown("**Underwater curve (drawdown %)**")
            st.area_chart(uw_df, use_container_width=True)

        # ---- Monthly & annual performance ----
        mdf = monthly_table(bt_summary, trades)
        if len(mdf):
            st.markdown("### Monthly Performance")
            st.dataframe(mdf, use_container_width=True, hide_index=True)
            st.download_button("Download Monthly (CSV)",
                               mdf.to_csv(index=False).encode(),
                               "monthly_performance.csv", "text/csv")
        adf = annual_table(bt_summary, trades)
        if len(adf):
            st.markdown("### Annual Performance")
            st.dataframe(adf, use_container_width=True, hide_index=True)

        # ---- Exposure analysis ----
        expo = exposure_report(bt_summary, settings)
        if expo:
            st.markdown("### Exposure Analysis")
            st.dataframe(_kv_table(expo), use_container_width=True, hide_index=True)

        # ---- Extended trade analytics ----
        eta = extended_trade_analytics(trades, bt_summary)
        if eta:
            st.markdown("### Extended Trade Analytics")
            st.dataframe(_kv_table(eta), use_container_width=True, hide_index=True)

        # ---- Capital timeline (daily) ----
        if daily_df is not None and len(daily_df):
            with st.expander("Capital Timeline (daily)", expanded=False):
                st.dataframe(daily_df, use_container_width=True, hide_index=True)
                st.download_button("Download Capital Timeline (CSV)",
                                   daily_df.to_csv(index=False).encode(),
                                   "capital_timeline.csv", "text/csv")

        # ---- Portfolio validation self-tests ----
        st.markdown("### Portfolio Validation Checks")
        vchecks = validation_checks(bt_summary, trades, settings)
        vdf = pd.DataFrame(vchecks)
        ...
        def _hl_v(val):
            ...
            return ""
        st.dataframe(_cellstyle(vdf.style, _hl_v, subset=["Result"]),
                     use_container_width=True, hide_index=True)
        if any(c["Result"] == "FAIL" for c in vchecks):
            st.error("One or more portfolio accounting checks FAILED.")
        else:
            st.success("All portfolio accounting checks passed.")
    else:
...
with tabs[3]:
    ...
    an, equity_curve, ordered_ex = compute_analytics(
        trades, results["engine_signals"], settings)
...
with tabs[4]:
    ...
    if results["engine_signals"]:
        ...
        audit_rows = []
        overall_fail = 0
        last_buy_by_symbol: Dict[str, str] = {}
        for s in results["engine_signals"]:
            k = (s.symbol, s.tracking_start_date, s.pullback_date, s.buy_date)
            prev_bd = last_buy_by_symbol.get(s.symbol)
            checks = audit_signal(s, results["data"][s.symbol],
                                  ...
                                  prev_buy_date=prev_bd)
            last_buy_by_symbol[s.symbol] = s.buy_date
        ...
        def _hl_audit(val):
            ...
            if val == "PASS":
                return "background-color: #ccffcc"
            return ""
        st.dataframe(_cellstyle(adf.style, _hl_audit), use_container_width=True)
        if overall_fail == 0:
...
with tabs[6]:
    ...
    # ---- Automated self-tests: module-consistency + portfolio accounting ----
    # Runs after every backtest. ANY failing row makes the whole run INVALID.
    st.markdown("### Automated Self-Tests")
    st.caption("Regression gate -- run automatically after every backtest; any "
               "FAIL invalidates the run.")

    _selftests = []

    def _stadd(name, ok, detail=""):
        _selftests.append({"Check": name,
                           "Result": "PASS" if ok else "FAIL",
                           "Detail": detail})

    # Module-consistency / regression gates
    _stadd("Strategy Engine == Verification Module (100%)",
           bool(verification_passed and mismatch_ct == 0),
           f"{match_ct} match / {mismatch_ct} mismatch")
    _stadd("Engine signal set == Verified signal set",
           engine_keys == verified_keys,
           f"{len(engine_keys)} engine / {len(verified_keys)} verified")
    _stadd("Backtest consumed only verified signals",
           backtest_consumed_keys.issubset(engine_keys),
           f"{len(backtest_consumed_keys)} consumed")

    # Portfolio-accounting self-tests (same suite as the Backtest tab)
    _selftests.extend(validation_checks(bt_summary, trades, settings))

    _st_df = pd.DataFrame(_selftests)
    ...
    st.dataframe(_cellstyle(_st_df.style, _hl_st, subset=["Result"]),
                 use_container_width=True, hide_index=True)

    _gate_pass = all(c["Result"] != "FAIL" for c in _selftests)
    if _gate_pass:
        st.success("All automated self-tests passed -- run is valid.")
    else:
        st.error("SELF-TESTS FAILED -- this run is INVALID. See the red rows above.")

    verdict = "PASS" if _gate_pass else "FAIL"
